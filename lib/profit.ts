export function calculateProfit(input) {
  ...
  const netProfit = grossSale - input.purchaseCost - input.shippingPaid - input.packagingCost - totalFees;
  ...
  return { grossSale, totalFees, netProfit, roi, margin, breakEvenPrice, weakProfit: netProfit < 15 || margin < 20 };
}
